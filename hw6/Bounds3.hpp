...
inline bool Bounds3::IntersectP(const Ray &ray, const Vector3f &invDir,
  ...
  auto xmin = (pMin.x - ray.origin.x) * invDir.x;
  auto xmax = (pMax.x - ray.origin.x) * invDir.x;
  if (dirIsNeg[0]) {
    std::swap(xmin, xmax);
  }

  auto tmin = xmin, tmax = xmax;

  auto ymin = (pMin.y - ray.origin.y) * invDir.y;
  auto ymax = (pMax.y - ray.origin.y) * invDir.y;
  if (dirIsNeg[1]) {
    std::swap(ymin, ymax);
  }
  ...
  auto zmin = (pMin.z - ray.origin.z) * invDir.z;
  auto zmax = (pMax.z - ray.origin.z) * invDir.z;
  if (dirIsNeg[2]) {
    std::swap(zmin, zmax);
  }
