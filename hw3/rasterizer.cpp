...
static bool insideTriangle(int x, int y,
                           const std::array<Eigen::Vector4f, 3UL> &v) {
  Vector2f p0(v[0].x(), v[0].y());
  Vector2f p1(v[1].x(), v[1].y());
  Vector2f p2(v[2].x(), v[2].y());
  Vector2f p3(x + 0.5f, y + 0.5f);
  auto cross = [](const Eigen::Vector2f &v1, const Eigen::Vector2f &v2) {
    return v1.x() * v2.y() - v1.y() * v2.x();
  };

  auto res0 = cross(p3 - p0, p0 - p1);
  auto res1 = cross(p3 - p1, p1 - p2);
  auto res2 = cross(p3 - p2, p2 - p0);
...
void rst::rasterizer::rasterize_triangle(
  ...
  auto v = t.toVector4(); // v是3个顶点的其次坐标表示
      ...
      if (insideTriangle(x, y, v)) {
        ...
        if (depth_buf[index] > z_interpolated) {
          ...
          auto interpolated_color =
              alpha * color[0] + beta * color[1] + gamma * color[2];

          const auto &normal = t.normal;
          auto interpolated_normal =
              alpha * normal[0] + beta * normal[1] + gamma * normal[2];

          const auto &texcoords = t.tex_coords;
